...
const MentoringModal = ({
  ...
  const [formData, setFormData] = useState({
    ...
    proposedLocation: "", // Only for in-person sessions
  });
  ...
  useEffect(() => {
    if (mode === "edit" && initialData) {
      setFormData({
        ...
        proposedLocation: initialData.proposedLocation || "",
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      (formData.sessionType === "in-person" && !formData.proposedLocation) // Only require location for in-person
    ) {
      toast({
        title: "Erreur",
    ...
    try {
      ...
      if (mode === "edit" && initialData) {
        ...
        toast.success("Session de mentorat mise à jour avec succès!");
      } else {
        response = await createTraining(trainingData);
        toast.success("Session de mentorat créée avec succès!");
      }
                const trainingsData = Array.isArray(response?.data) ? response.data : [];

      onSuccess(trainingsData);
      ...
      <DialogContent className="!max-w-4xl !max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2 text-tacir-darkblue">
            ...
            <span>
              {mode === "edit"
                ? "Modifier la session de mentorat"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className="border-tacir-lightgray focus:border-tacir-lightblue">
                    <SelectValue placeholder="Ajouter un mentor" />
                  </SelectTrigger>
                  <SelectContent>
                    {mentors.map((mentor) => (
            ...
            <div className="space-y-4 mt-4">
              ...
              {/* Only show location field for in-person sessions */}
              {formData.sessionType === "in-person" && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: TACIR Lab, Coworking Space..."
                    className="border-tacir-lightgray focus:border-tacir-lightblue"
                  />
                </div>
              )}
